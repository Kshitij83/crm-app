...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    try {
      const rules = await OpenAIService.parseNaturalLanguageRules({ description });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    res.json({
      ...
      input: description,
      output: rules,
      note: "These rules may be generated by the mock service if no OpenAI API key is configured."
    });
  })
);
